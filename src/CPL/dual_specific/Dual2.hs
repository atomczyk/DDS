...
applyDualrule1 [] = []
applyDualrule1 (x:xs) = if atomicDualseq x then (x:applyDualrule1 xs) else (applyDualRules x) ++ applyDualrule1 xs
...
dualprooftree (Node (x, []) [])
 | all atomicDualseq x = prooftree2x (Node ( x, []) [])
 | not (all atomicDualseq x) && (resolution_poss2 (Node (x, []) [])) = (prooftree2x (Node (x, []) []))
 | otherwise = (Node (x, []) [(Node ((applyDualrule1 x), []) [])])
dualprooftree (Node (x, us) xs) = Node (x, us) (map dualprooftree xs)


{-
*Dual2> derivationF (((V 1 `A` V 2) `A` V 3) `I` (V 1 `A` (V 2 `A` V 3)) `D` V 5)
Node {rootLabel = ([Dual ([],[(((V 1 `A` V 2) `A` V 3) `I` (V 1 `A` (V 2 `A` V 3))) `D` V 5],[])],[]), subForest = [Node {rootLabel = ([Dual ([],[((V 1 `A` V 2) `A` V 3) `I` (V 1 `A` (V 2 `A` V 3))],[]),Dual ([V 5],[],[])],[]), subForest = [Node {rootLabel = ([Dual ([],[N ((V 1 `A` V 2) `A` V 3)],[]),Dual ([],[],[V 1 `A` (V 2 `A` V 3)]),Dual ([V 5],[],[])],[]), subForest = [Node {rootLabel = ([Dual ([],[N (V 1 `A` V 2)],[]),Dual ([N (V 3)],[],[]),Dual ([V 1],[],[V 2 `A` V 3]),Dual ([V 5],[],[])],[]), subForest = [Node {rootLabel = ([Dual ([N (V 1)],[],[]),Dual ([N (V 2)],[],[]),Dual ([N (V 3)],[],[]),Dual ([V 3,V 2,V 1],[],[]),Dual ([V 5],[],[])],[]),
 subForest = [Node {rootLabel = ([Dual ([V 2,V 1],[],[]),Dual ([V 5],[],[]),Dual ([V 3,V 2,V 1],[],[]),Dual ([N (V 3)],[],[]),Dual ([N (V 1)],[],[]),Dual ([N (V 2)],[],[])],[(Dual ([V 3,V 2,V 1],[],[]),Dual ([V 5],[],[]))]), subForest = []}]}]}]}]}]}
...
--(Node (x, []) [(Node ((applyDualrule1 x), []) [])])

{-
Node {rootLabel = ([Dual ([],[N ((V 1 `A` V 2) `A` N (V 1 `A` V 2))],[])],[]), subForest = [Node {rootLabel = ([Dual ([],[N (V 1 `A` V 2)],[]),Dual ([],[],[N (N (V 1 `A` V 2))])],[]), subForest = []}]}
-}
...
derivation x = until (der_condx) dualprooftree (Node (x,[]) [])

{-
(derivationF (N ((V 1 `A` V 2) `A` N (V 1 `A` V 2))))


Node {rootLabel = ([Dual ([],[N ((V 1 `A` V 2) `A` N (V 1 `A` V 2))],[])],[]), subForest = [Node {rootLabel = ([Dual ([],[N (V 1 `A` V 2)],[]),Dual ([],[],[N (N (V 1 `A` V 2))])],[]), subForest = [Node {rootLabel = ([Dual ([],[],[]),Dual ([],[N (V 1 `A` V 2)],[]),Dual ([],[],[N (N (V 1 `A` V 2))])],[(Dual ([],[N (V 1 `A` V 2)],[]),Dual ([],[],[N (N (V 1 `A` V 2))]))]), subForest = []}]}]}

-}

-- | Using an above function derivation, derivation tree is returned for a singular formula. (hipersekwent, pamięć)
...
returncompl [] zs = []
returncompl xs [] = []
returncompl (x:xs) zs = if any (\z -> elem z zs) (compl x) then x:(filter (\y -> elem y zs) (compl x))  else returncompl xs zs
--returncompl (x:xs) zs = if any (\z -> elem z zs) (compl x) then x:(compl x):[] else returncompl xs zs
...
fun (x1:x2:xs) (Dual (z1, z2, z3)) (Dual (y1, y2, y3)) = Dual ((rmdups (filter (/= x1) z1 ++ filter (/= x2) y1)), (rmdups (filter (/= x1) z2 ++ filter (/= x2) y2)), (rmdups (filter (/= x1) z3 ++ filter (/= x2) y3)))
...
isTaut :: For -> Bool
isTaut x = closed_tree (derivationF x)
...
--example test
--Node ([Dual ([],[N ((V 1 `A` V 2) `A` N (V 1 `A` V 2))],[])],[]) []

--Node {rootLabel = ([Dual ([],[N ((V 1 `A` V 2) `A` N (V 1 `A` V 2))],[])],[]),
--subForest = [Node {rootLabel = ([Dual ([],[N (V 1 `A` V 2)],[]),Dual ([],[],[N (N (V 1 `A` V 2))])],[]), subForest = []}]}



-- ~~~~(p v ~p)
resEx = N (N (N (N (V 1 `D` N (V 1)))))
example = N (N ((V 1 `D` N (V 1)) `A` N (V 1 `D` N (V 1))))
for = N ((A (V 1) (V 2)) `A` N ((A (V 1) (V 2))))
