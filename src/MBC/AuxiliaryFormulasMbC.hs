module AuxiliaryFormulasMbC where
  ...
  (x:xs) -> case (isAtomic x) of
      ...
      where
        ...
        formulaComplexity x = case x of
          ...
          V _       -> 0
          N (V _)   -> 0
          N (N y)   -> formulaComplexity x + 1--modyfikacja bez 1
          E y z     -> 1 + formulaComplexity (I y z) + formulaComplexity (I z y)
          I y z     -> 1 + formulaComplexity (N y) + formulaComplexity z
          A y z     -> 1 + formulaComplexity y + formulaComplexity z
          D y z     -> 1 + formulaComplexity y + formulaComplexity z
          N (E y z) -> 1 + formulaComplexity (N (I y z)) + formulaComplexity (N (I z y))
          N (I y z) -> 1 + formulaComplexity y + formulaComplexity (N z)
          N (A y z) -> 1 + formulaComplexity (N y) + formulaComplexity (N z)
          N (D y z) -> 1 + formulaComplexity (N y) + formulaComplexity (N z)
        isAtomic :: For -> Bool
        isAtomic for = case for of
          Verum   -> True
          V _     -> True
...
-- | Quicksort a list of formulas with respect to their complexity. Alpha/beta version
sortComplexity :: [For] -> [For]
sortComplexity fs = sortBy order $ filter (not . isAtomic) fs
  where order x y
          | cx < cy   = LT
          | cx > cy   = GT
          | otherwise = EQ
          where cx = formulaComplexity x
                cy = formulaComplexity y
