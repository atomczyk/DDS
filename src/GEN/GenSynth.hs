...
michal = do
  ...
  let chunks = chunksOf 350 content
  let chunk1 = chunks !! 0
  let chunk2 = chunks !! 1
  let chunk3 = chunks !! 2
  let chunk4 = chunks !! 3
  ...
  let chunk8 = chunks !! 7
  let chunk9 = chunks !! 8
  let chunk10 = chunks !! 9
  let info1 = concat (map mainFunctionMichal chunk1)
  let info2 = concat (map mainFunctionMichal chunk2)
  let info3 = concat (map mainFunctionMichal chunk3)
  let info4 = concat (map mainFunctionMichal chunk4)
  let info5 = concat (map mainFunctionMichal chunk5)
  let info6 = concat (map mainFunctionMichal chunk6)
  let info7 = concat (map mainFunctionMichal chunk7)
  let info8 = concat (map mainFunctionMichal chunk8)
  let info9 = concat (map mainFunctionMichal chunk9)
  let info10 = concat (map mainFunctionMichal chunk10)
  let infoGlobal = concat (map mainFunctionMichal content)
  let info1Global = take 1 infoGlobal
  let columns = "formula" ++ ", " ++ "good_instruction" ++ ", " ++ "is_taut" ++ ", " ++ "num_of_nodes" ++ ", " ++ "num_of_branches" ++ ", " ++ "b_depth\n"
  writeFile "data/part1.txt" columns
  appendFile "data/part1.txt" $ unlines $ infoGlobal
  print (map showInstructions (map exportIndex11 (filterRules [V 0, V 1] [[V 0, V 1], [V 0, V 1]])))
--  print (take 1 fors)
...
{-
 ...
 -- let info1 = take 1 info
  let columns = "formula" ++ ", " ++ "good_instruction" ++ ", " ++ "is_taut" ++ ", " ++ "num_of_nodes" ++ ", " ++ "num_of_branches" ++ ", " ++ "b_depth\n"
  writeFile "data/part1.txt" columns
  appendFile "data/part1.txt" $ unlines $ info
--  print (take 1 fors)
--  print (adjustFormulaIns (((V 0) `I` (V 0)) `I` (V 1)) [[V 0],[V 1, V 1], [V 2, V 2, V 2, V 2]])
--  print result
--  let ins = [[V 1],[V 3, V 3], [V 4, V 4, V 4, V 4], [V 5, V 5, V 5, V 5,V 5, V 5, V 5, V 5], [V 2, V 2, V 2, V 2,V 2, V 2, V 2, V 2,V 2, V 2, V 2, V 2,V 2, V 2, V 2, V 2] ]
...
--    instruction       = adjustFormulaIns for (parseI (tail (dropWhile (\x -> x /= ',') x)))
...
--    goodInstructions  = map showInstructions (map exportIndex11 (filterRules base instruction))
    goodInstructions  = filterRules (genBase for) instruction
 --   goodInstructions1 = map (\x -> adjustFormulaIns for x) goodInstructions
 --   base              = adjustFormulaIns for [V 0, V 1, V 2, V 3, V 4]
...
--  let info1 = take 1 info
  ...
  appendFile "data/decision_tree_25_new.txt" $ unlines $ info
--  print "Done"
--  print (take 1 fors)
